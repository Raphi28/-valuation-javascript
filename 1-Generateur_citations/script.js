...
// Cette fonction calcule et retourne un nombre aléatoire entre un minimum et un maximum (définis dans la fonction generateQuote())
function nombreAleatoire(min,max) {
    return Math.random() * (max-min) + min
}

// Cette fonction affiche une citation différente lorsque le bouton est cliqué cependant la citation ne change pas tout le temps
function generateQuote() {
    for (let index = 0; index < data.length; index++) {
        let nombreRandom = parseInt(nombreAleatoire(1,data.length))
        if (nombreRandom != index++) {
            if (nombreRandom === parseInt(index)) {
                console.log("test réussi");
                let afficheCitation = document.querySelector("#citation")
                afficheCitation.textContent = data[index]
...
// Au clique, on déclenche la fonction generateQuote()
